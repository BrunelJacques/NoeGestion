...
def DatetimeToIso(date_):
    return DatetimeToStr(date_,iso=True)
        ...
        if isinstance(dte,datetime.datetime):
            ...
            if hh + mn >0:
                deltaheure = ' %s:%s'%(("00"+str(hh))[-2:],
                                        ("00" + str(mn))[-2:])
        if iso: return "%s-%s-%s"%(yyyy,mm,dd) + deltaheure
        else: return "%s/%s/%s"%(dd,mm,yyyy)+ deltaheure
    else: return str(dte)
...
def FinDeMois(date_):
    ...
    if isinstance(date_,str):
        ...
        if len(date_.split('/')) == 3:
            dtretour = DatetimeToStr(dtretour) + deltaheure
...
def DebutDeMois(date_):
    ...
    if isinstance(date_,str):
        if len(date_.split('/')) == 3:
            dtretour = DatetimeToStr(dtretour)
        elif len(date_.split(('-'))) == 3:
